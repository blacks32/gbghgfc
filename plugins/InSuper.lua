...
local function check_member_super(cb_extra, success, 
...
result)
  ...
  if success == 0 then
	send_large_msg(receiver, "Promote me to admin 
...
local function check_member_superrem

(cb_extra, success, result)
  local receiver = cb_extra.receiver
  local data = cb_extra.data
  local msg = cb_extra.msg
  for k,v in pairs(result) do
...
[tostring(groups)][tostring(msg.to.id)] = nil
      save_data(_config.moderation.data, data)
	  local text = 'SuperGroup has been removed'
      return reply_msg

(msg.id, text, ok_cb, false)
    end
  end
end
...
superrem(msg)
    ...
    channel_get_users(receiver, check_member_superrem,{receiver = 

receiver, data = data, msg = msg})
end
...
string.gsub(cb_extra.msg.to.print_name, "_", " ")
...
title..admin_num..user_num..kicked_num..channel_id..channel_username
    send_large_msg(cb_extra.receiver, text)
end
...
callback_who(cb_extra, success, result)
...
io.open("./groups/lists/supergroups/"..cb_extra.receiver..".txt", "w")
    file:write(text)
    file:flush()
    file:close()
    send_document

(cb_extra.receiver,"./groups/lists/supergroups/"..cb_extra.receiver..".txt", ok_cb, false)
	post_msg(cb_extra.receiver, text, ok_cb, false)
end

--Get and output list 

of kicked users for supergroup
local function callback_kicked(cb_extra, success, result)
--vardump(result)
local text = "Kicked Members for SuperGroup 
...
local function lock_group_links(msg, data, target)
  ...
  local group_link_lock = data[tostring(target)]
...
local function unlock_group_links
...
(msg.id,"> #Link posting has been #unlocked", ok_cb, false)
else
...
then
  ...
  local group_spam_lock = data[tostring(target)]
...
(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget
...
function unlock_group_spam(msg, data, target)
  if not is_momod(msg) then
    return
  end
...
group_spam_lock == 'no' then
  	local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg

(msg.id,"> SuperGroup #spam is #not locked", ok_cb, false)
  else
...
reply_msg(msg.id,"> SuperGroup #spam has been #unlocked", ok_cb, false)
else
...
local function unlock_group_flood(msg, data, target)
  ...
  local group_flood_lock = data[tostring(target)]
...
reply_msg(msg.id,"> #Spamming has been #unlocked", ok_cb, false)
else
...
'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Arabic/Persian has been #locked", ok_cb, 
...
local function lock_group_tag(msg, data, target)
  ...
  local group_tag_lock = data[tostring

(target)]['settings']['lock_tag']
...
local function unlock_group_webpage(msg, data, target)
  ...
  local group_webpage_lock = data[tostring(target)]

['settings']['lock_webpage']
...
(target)]['settings']['lock_webpage'] = 'no'
    save_data(_config.moderation.data, data)
    local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget
...
local function lock_group_fwd(msg, data, target)
  ...
  local group_fwd_lock = data[tostring(target)]
...
(msg.id,"> #Forward Msg has been #locked", ok_cb, false)
else
...
[tostring(target)]['settings']['lock_badw'] = 'yes'
    save_data(_config.moderation.data, data)
    local hash = 'group:' ..msg:to.id
  	   local group_lang = 
...
function unlock_group_badw(msg, data, target)
  if not is_momod(msg) then
    return
  end
...
group_badw_lock == 'no' then
  	local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg

(msg.id,"> #Badwords is #already unlocked", ok_cb, false)
  else
...
reply_msg(msg.id,"> #Badwords has been #unlocked", ok_cb, false)
else
    return 'قفل فش باز شد'
  end
end
-- lock emoji Fanction by MehdiHS!
local function lock_group_emoji(msg, 
...
local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Emoji is #already 
...
[tostring(target)]['settings']['lock_emoji'] = 'no'
    save_data(_config.moderation.data, data)
    local hash = 'group:' ..msg:to.id
  	   local group_lang = 
...
English Fanction by MehdiHS!
local function lock_group_eng(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_eng_lock = data[tostring(target)]
...
local function unlock_group_eng(msg, 
...
local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #English is #already 

unlocked", ok_cb, false)
  else
...
(msg.id,"> #English has been #unlocked", ok_cb, false)
else 
...
..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> SuperGroup #members has been #unlocked", ok_cb, 
...
local function lock_group_rtl(msg, data, target)
  ...
  local group_rtl_lock = data[tostring

(target)]['settings']['lock_rtl']
...
local function lock_group_tgservice(msg, data, target)
  ...
  local group_tgservice_lock = data[tostring

(target)]['settings']['lock_tgservice']
...
local function unlock_group_tgservice(msg, data, target)
  ...
  local group_tgservice_lock = data[tostring(target)]
...
group_sticker_lock == 'yes' then
  	local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return 

reply_msg(msg.id,"> #Sticker posting is #already locked", ok_cb, false)
  else
...
hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Sticker posting is #already 
...
(msg.id,"> #Sticker posting has been #unlocked", ok_cb, false)
else
...
'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Contact posting is #already locked", 
...
local hash = 'group:' ..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Contact posting has been 
...
group_contacts_lock = data[tostring(target)]['settings']['lock_contacts']
...
..msg:to.id
  	   local group_lang = redis:hget(hash,'lang')
  	   if group_lang then
    return reply_msg(msg.id,"> #Contact posting has been #unlocked", ok_cb, 
...
data[tostring(target)]['settings']['strict']
  if group_strict_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,"> #Settings will be 

#strictly_enforced", ok_cb, false)
  end
end
...
data[tostring(target)]['settings']['strict']
  if group_strict_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,"> #Settings will #not be strictly 

enforced", ok_cb, false)
  end
end
...
return
  ...
  save_data(_config.moderation.data, data)
  return reply_msg(msg.id,"*SuperGroup rules 
...
local function get_rules(msg, data)
 ...
 return reply_msg(msg.id,"*No rules available.", ok_cb, false)
  ...
  local group_name = data[tostring(msg.to.id)]
...
function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return reply_msg(msg.id,"*For moderators only!", ok_cb, false)
  end
  local 

group_public_lock = data[tostring(target)]['settings']['public']
...
عمومی بوده است'
  ...
  return reply_msg(msg.id,"*SuperGroup is now: 
...
local function unset_public_membermod(msg, data, target)
  ...
  local group_public_lock = data[tostring
...
[tostring(target)]['settings']['public'] = 'no'
	data[tostring(target)]['long_id'] = msg.to.long_id
    save_data(_config.moderation.data, data)
    return reply_msg

(msg.id,"*SuperGroup is now: not public",ok_cb,false)
  end
end
...
['settings']['flood_msg_max'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']
...
data[tostring(target)]['settings'] then
...
['lock_rtl'] = 'no'
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring

(target)]['settings']['lock_emoji'] then
...
(target)]['settings']['lock_audio'] then
...
['lock_member'] then
	...
	if data[tostring(target)]['settings'] 
...
local function demote_admin(receiver, member_username, 
...
[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..' is not a moderator.')
  end
  data[group]['moderators'][tostring
...
[group] then
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg

(receiver, member_username..' is already a moderator.')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
...
['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
...
function get_message_callback(extra, success, 
...
elseif get_cmd == 'id' and result.action then
		...
		if action == 'chat_add_user' or action == 'chat_del_user' or 

action == 'chat_rename' or action == 'chat_change_photo' then
...
(msg.to.id, name_log.." ["..msg.from.id.."] obtained id for msg fwd from: ["..result.fwd_from.peer_id.."]")
		id2 = send_large_msg(channel, 
...
channel_id = result.to.peer_id
    if member_id == msg.from.id then
      return send_large_msg("channel#id"..channel_id, "Leave using kickme command")
    end
    if 

is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "You can't kick 
...
'channel_block' and result.action and result.action.type == 'chat_add_user' then
...
"user#id"..user_id, ok_cb, false)
		...
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] set: ["..user_id.."] as 
...
local channel_id = "channel#id"..result.to.peer_id
		if is_admin2(result.from.peer_id) then
			return send_large_msg(channel_id, "You can't 

demote global admins!")
		end
...
channel_id = 'channel#id'..result.to.peer_id
...
user_id = "user#id"..result.from.peer_id
			channel_set_admin(channel_id, user_id, ok_cb, false)
			data[tostring

(result.to.peer_id)]['set_owner'] = tostring(result.from.peer_id)
			save_data(_config.moderation.data, data)
			savelog

(msg.to.id, name_log.." ["..msg.from.id.."] set: ["..result.from.peer_id.."] as owner by reply")
			if result.from.username then
...
"..result.from.peer_id.." ] added as owner"
			end
			send_large_msg(channel_id, text)
		end
	elseif get_cmd == 
...
("channel#id"..result.to.peer_id, member_username, member_id)
...
member_username = member_name:gsub("_", " ")
    if result.from.username then
		member_username = '@'.. result.from.username
    end
		local member_id 
...
= result.from.peer_id
		...
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] demoted mod: 

@"..member_username.."["..user_id.."] by reply")
		demote2("channel#id"..result.to.peer_id, member_username, member_id)
		--channel_demote

(channel_id, user, ok_cb, false)
...
if get_cmd == "demoteadmin" then
		if is_admin2(result.peer_id) then
			return send_large_msg(receiver, "You can't demote global 
...
user_id = "user#id"..result.peer_id
...
("channel#id"..channel_id, "Leave using kickme command")
    end
...
("channel#id"..channel_id, "You can't kick other admins")
    end
		kick_user(user_id, channel_id)
...
[tostring(channel)]['set_owner'] = tostring(result.peer_id)
			save_data(_config.moderation.data, data)
			savelog(channel, 

name_log.." ["..from_id.."] set ["..result.peer_id.."] as owner by username")
		if result.username then
			text = member_username.."> [ 

"..result.peer_id.." ] added as owner"
		else
...
(receiver, member_username, user_id)
	elseif get_cmd == "demoteadmin" then
		local user_id = "user#id"..result.peer_id
		local channel_id = 
...
extra.channel
	...
	channel_demote(channel_id, user_id, ok_cb, false)
		if result.username then
			text = "@"..result.username.." has been demoted from 
...
if get_cmd == "channel_block" then
 ...
 if vusername == member or vpeer_id == memberid then
  ...
  if user_id == sender then
      ...
      if is_momod2(user_id, channel_id) and 

not is_admin2(sender) then
        return send_large_msg("channel#id"..channel_id, "You can't kick mods/owner/admins")
      end
...
["..msg.from.id.."] kicked: @"..v.username.." ["..v.peer_id.."]")
      else
        text = ""
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked: 
...
"channel#id"..cb_extra.msg.to.id
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] 

has been set as an admin"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "> 

["..v.peer_id.."] has been set as an admin"
...
'setowner' then
	for k,v in pairs(result) do
		vusername = v.username
		vpeer_id = tostring(v.peer_id)
		if vusername == member or vpeer_id == 
...
group_owner = data[tostring(channel)]['set_owner']
			if group_owner then
				if not is_admin2(tonumber(group_owner)) and not 

is_support(tonumber(group_owner)) then
					local user = "user#id"..group_owner
					channel_demote

(receiver, user, ok_cb, false)
				end
					local user_id = "user#id"..v.peer_id
...
channel_set_admin(receiver, user_id, ok_cb, false)
		...
		save_data(_config.moderation.data, data)
					savelog(channel, name_log.."["..from_id.."] set ["..v.peer_id.."] as 
...
[tostring(channel)]['set_owner'] = tostring(memberid)
				save_data(_config.moderation.data, data)
				savelog

(channel, name_log.."["..from_id.."] set ["..memberid.."] as owner by username")
...
set_supergroup_photo(msg, success, result)
  local data = load_data(_config.moderation.data)
...
'*Failed, please try again!', ok_cb, false)
  end
end
...
"..msg.to.print_name.."("..msg.to.id..") added")
...
(msg.id, 'SuperGroup is not added.', ok_cb, false)
			end
...
"*no owner,ask admins in support groups to set owner for your SuperGroup"
			end
			savelog(msg.to.id, name_log.." 

["..msg.from.id.."] used /owner")
...
'Bots'
			...
			channel_get_bots(receiver, callback, 

{receiver = receiver, msg = msg, member_type = member_type})
		end
...
savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested Kicked users list")
			channel_get_kicked(receiver, callback_kicked, {receiver = 
...
cbreply_extra = {
			...
			get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'kick' and string.match(matches[2], '^%d+
...
(user_id, channel_id) and not is_admin2(user_id) then
...
cbreply_extra = {
	...
	get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif type(msg.reply_id) ~= "nil" and matches[2] == "from" and is_momod(msg) 
...
matches[1] == 'kickme' then
			if msg.to.type == 'channel' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] left 
...
(msg.to.id)]['settings']['set_link'] = 'waiting'
			save_data(_config.moderation.data, data)
			return 'Please send the new 

group link now!'
		end

		if msg.text then
			if msg.text:match("^(https://telegram.me/joinchat/%S+)$") and data[tostring

(msg.to.id)]['settings']['set_link'] == 'waiting' and is_owner(msg) then
...
group_link = data[tostring(msg.to.id)]['settings']['set_link']
			if not group_link then
				return "> Create a link using /newlink 

first!\n\nOr if I am not creator use /setlink to set your link"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested 
...
["..msg.from.id.."] invited @"..username)
			resolve_username(username,  callbackres, cbres_extra)
		end

		if matches[1] == 'res' 

and is_owner(msg) then
			local cbres_extra = {
				channelid = msg.to.id,
				get_cmd = 'res'
...
["..msg.from.id.."] resolved username: @"..username)
			resolve_username(username,  callbackres, cbres_extra)
		end

		if matches[1] 

== 'kick' and is_momod(msg) then
...
(msg.from.id) and not is_owner(msg) then
					return
				end
			if type(msg.reply_id) ~= "nil" 
...
'setadmin' and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				local user_id = 

"user#id"..matches[2]
				local get_cmd = 'setadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd 
...
["..msg.from.id.."] set admin @"..username)
				resolve_username(username, callbackres, cbres_extra)]]
				local	get_cmd 
...
string.gsub(matches[2], '@', '')
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, 
...
username=username})
		...
		if matches[1] == 'demoteadmin' then
			if not is_support(msg.from.id) and not 
...
cbreply_extra = {
			...
			elseif matches[1] == 'demoteadmin' and 

string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
...
["..msg.from.id.."] demoted admin @"..username)
				resolve_username(username, callbackres, cbres_extra)
			end
		end
...
setowner = get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'setowner' and string.match(matches[2], '^%d+$') 
...
(group_owner) and not is_support(group_owner) then
						channel_demote(receiver, user_id, ok_cb, false)
...
(receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'setowner' and not string.match
...
username = matches[2]
				local username = string.gsub(matches[2], '@', '')
				channel_get_users (receiver, 

in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, username=username})
			end
		end
...
string.match(matches[2], '^%d+$') then
			...
			user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'promote' and not 
...
matches[1] == 'demote' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local 
...
name_log.." ["..msg.from.id.."] renamed SuperGroup to: "..matches[2])
...
if msg.service and msg.action.type == 'chat_rename' then
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] renamed SuperGroup to: 
...
[data_cat] = about_text
			save_data(_config.moderation.data, data)
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] set 

SuperGroup description to: "..about_text)
			channel_set_about(receiver, about_text, ok_cb, false)
			return "Description has been 
...
ok_username_cb (extra, success, result)
				local receiver = extra.receiver
				if success == 1 then
	...
	send_large_msg(receiver, "SuperGroup username Set.\n\nSelect the chat again to see the changes.")
				...
				send_large_msg(receiver, "Failed to set SuperGroup username.\nUsername may already be taken.\n\nNote: Username can use a-z, 0-9 and 
...
["..msg.from.id.."] has changed group rules to ["..matches[2].."]")
...
["..msg.from.id.."] started setting new SuperGroup photo")
...
== 'clean' then
		...
		if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return reply_msg(msg.id,"No moderator(s) in this 

SuperGroup!", ok_cb,false)
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
...
(msg.id,"Modlist has been cleaned!", ok_cb,false)
			end
...
(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." 

["..msg.from.id.."] cleaned about")
				channel_set_about(receiver, about_text, ok_cb, false)
				return reply_msg

(msg.id,"About has been Cleaned", ok_cb,false)
			end
...
reply_msg(msg.id,"Mutelist Cleaned", ok_cb,false)
			end
...
success == 0 then
				...
				local username = ""
				channel_set_username(receiver, username, 

ok_username_cb, {receiver=receiver})
			end
		    if matches[2] == "bots" and is_momod(msg) then
            savelog(msg.to.id, name_log.." 

["..msg.from.id.."] kicked all SuperGroup bots")
...
== 'gbanlist' and is_sudo then 
            local hash = 'gbanned'
             ...
             save_data(_config.moderation.data, data)
                redis:del(hash)
			return reply_msg(msg.id,"GbanList Have Been Cleaned!", 
...
(msg.to.id, name_log.." ["..msg.from.id.."] locked Emoji ")
...
name_log.." ["..msg.from.id.."] locked Tgservice Actions")
...
(msg.to.id, name_log.." ["..msg.from.id.."] locked contact posting")
...
name_log.." ["..msg.from.id.."] locked gif posting")
...
name_log.." ["..msg.from.id.."] unlocked Forward Msg")
...
(msg.to.id, name_log.." ["..msg.from.id.."] unlocked RTL chars. in names")
				return unlock_group_rtl(msg, data, target)
			

end
				if matches[2] == 'tgservice' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked tgservice 
...
posting")
		...
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked audio posting")
			    ...
			    if matches[2] == 'document' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked document 
...
posting")
		...
		if not is_momod(msg) then
			...
			if tonumber(matches[2]) < 2 or tonumber
	...
	if matches[1] == 'public' and is_momod(msg) then
		...
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: public")
			...
			if matches[2] == 'no' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: 
...
SuperGroup to: mute "..msg_type)
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': 
...
["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return 

msg_type.." has been muted"
				else
					return "SuperGroup mute "..msg_type.." is already on"
...
is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
...
== 'documents' then
		...
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
			...
			return msg_type.." have been muted"
					...
					return "SuperGroup mute "..msg_type.." 
...
"Mute "..msg_type.." is already on"
		...
		if matches[1] == 'unmute' and is_momod(msg) 
...
then
	...
	if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute 
...
["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return 

msg_type.." has been unmuted"
				else
					return "Mute "..msg_type.." is already off"
...
savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
			...
			return msg_type.." have been unmuted"
					...
					return "Mute "..msg_type.." is already 
...
"..msg_type.." is already disabled"
				end
			end
		end


		if matches[1] == "muteuser" and is_momod(msg) 
...
"muteuser" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, 

user_id) then
					unmute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] 

removed ["..user_id.."] from the muted users list")
...
["..msg.from.id.."] added ["..user_id.."] to the muted users list")
					return reply_msg(msg.id,"> ["..user_id.."] added to the muted 

user list",ok_cb,false)
				end
	...
	local receiver = get_receiver(msg)
		...
		local username = string.gsub(matches[2], '@', '')
				resolve_username(username, callbackres, {receiver = receiver, get_cmd = 
...
matches[1] == 'settings' and is_momod(msg) then
			local target = msg.to.id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] 

requested SuperGroup settings ")
...
matches[1] == 'help' and not is_momod(msg) then
...
\n#porn [text]\nجستجو در 7 سایت +\n➖➖➖➖➖➖\n#time\nدریافت زمان دقیق!\n➖➖➖➖➖➖\n#support \nدریافت لینک گروه پشتیبانی!\n➖➖➖➖➖➖\n#setowner [reply, username]\nست کردن کاربر به عنوان خریدار گروه\n#promote [username|id]\nارتقاء مقام کاربر به ناظم 

گروه\n#demote [username|id]\nخلع مقام کردن کاربر از سمت ناظم ها\n➖➖➖➖➖➖\n#setname [text]\nتغییر اسم گروه\n#setphoto\nجایگزین کردن عکس گروه\n#setrules [text]\nگذاشتن قوانین برای گروه\n#setabout [text]\nگذاشتن متن توضیحات برای سوپر گروه(این متن در بخش توضیحات 

گروه هم نمایش داده میشه)\n➖➖➖➖➖➖\n#newlink\nساختن لینک جدید\n#link\nگرفتن لینک\n#linkpv\nارسال لینک گروه در پیوی شما!\n➖➖➖➖➖➖\n#rules\nنمایش قوانین\n➖➖➖➖➖➖\n#lock [links|flood|spam|Arabic|member|rtl|sticker|TgService|contacts|

forward|badword|emoji|english|tag|webpage|strict]\nقفل کردن لینک گروها-اسپم-متن و اسم های بزرگ -زبان فارسی-تعداد اعضا-کاراکتر های غیر عادی-استیکر-مخاطبین-فروارد-فوش-اموجی-انگلیسی-تگ-لینک سایت\n\nدقت کنید اگر گذینه اخری strict روشن باشد کاربر از گروه کیک میشود و پیغام پاک میشه در غیر این صورت فقط پیغام پاک 

میشود\n➖➖➖➖➖➖\n#unlock [links|flood|spam|Arabic|member|rtl|sticker|contacts|TgService|strict|forward|badword|emoji|english]\nباز کردن قفل امکانات بالا\n➖➖➖➖➖➖\n#mute [all|audio|gifs|photo|

video|text]\nپاک کردن سریع همه پیغام ها-عکس ها-گیف ها-صدا های ضبط شده-فیلم-متن\n➖➖➖➖➖➖\n#unmute [all|audio|gifs|photo|video|text]\nباز کردن قفل امکانات بالا\n➖➖➖➖➖➖\n#setflood [value]\nگذاشتن value به عنوان حساسیت اسپم\n➖➖➖➖➖➖\n#settings\nنمایش تنظیمات 

گروه\n➖➖➖➖➖➖\n#muteslist\nنمایش نوع پیغام های سایلنت شده\n*A \"muted\" message type is auto-deleted if posted\n➖➖➖➖➖➖\n#muteuser [username]\nسایلنت کردن یک کاربر خاص در گروه\nفقط خریدار (owner) میتونه کسیو سایلنت کنه ولی ناظم ها (Mods) میتونند فرد 

را از سایلنتی در بیاورند\n➖➖➖➖➖➖\n#mutelist\nنمایش لیست افراد سایلنت شده\n➖➖➖➖➖➖\n#clean [rules|about|modlist|mutelist|bots]\nپاک کردن لیست ناظم ها-درباره-لیست سایلنت شده ها-قوانین-بات ها\n➖➖➖➖➖➖\n#log\nبرگرداندن تاریخچه گروه در یک فایل متنی\n➖➖➖➖➖➖\n#version\nنمایش ورژن و ادمین های ربات!
...
فرد از گروه به صورت دائمی\n#unban @username\nخارج کردن یک فرد از حالت اخراج دائمی!\n#banlist\nلیست افراد بن شده.\n➖➖➖➖➖➖\n#info\nنمایش اطلاعات اصلی گروه\n➖➖➖➖➖➖\n#del [reply|number]\nپاک کردن تعداد پیام های مورد نظر با ریپلی و تعداد!\n➖➖➖➖➖➖\n#topstats\nنشان دادن ۳ نفر از فعال ترین اعضای 
...
red|blue|yellow|pink|orange|brown] )\nتبدیل متن شما به استیکر ...\n➖➖➖➖➖➖\n#photo [reply]\nبا ریپلای کردن استیکر میتوانید آن را به عکس تبدیل کنید!\n➖➖➖➖➖➖\n#vc [kalame](زبان ها : Farsi,En)\nتبدیل کلمه به صدا\n➖➖➖➖➖➖\n#weather [اسم شهر]\nدریافت اطلاعات آب و 

هوای یک منطقه\n➖➖➖➖➖➖\n#aparat [کلمه] \nجستوجو در آپارات!\n➖➖➖➖➖➖\n#me\nنمایش تعداد پیام های ارسال شده از شما\n➖➖➖➖➖➖\n#qr [کلمه]\nتبدیل کلمه،لینک،... شما به بارکد\n➖➖➖➖➖➖\n#insta [id, Post Link]\nدریافت اطلاعات ایدی و ... از اینستاگرام!\n➖➖➖➖➖➖\n#write [text]\nطراحی کلمه مورد تظر با 17 

فونت!\n➖➖➖➖➖➖\n#calc [2*2]\nمحاسبه جمع تفریق ضرب و...\n➖➖➖➖➖➖\n#porn [text]\nجستجو در 7 سایت +\n➖➖➖➖➖➖\n#time\nدریافت زمان دقیق!\n➖➖➖➖➖➖\n#support \nدریافت لینک گروه پشتیبانی!\n➖➖➖➖➖➖\n#setowner [reply, username]\nست کردن کاربر به عنوان خریدار 

گروه\n#promote [username|id]\nارتقاء مقام کاربر به ناظم گروه\n#demote [username|id]\nخلع مقام کردن کاربر از سمت ناظم ها\n➖➖➖➖➖➖\n#setname [text]\nتغییر اسم گروه\n#setphoto\nجایگزین کردن عکس گروه\n#setrules [text]\nگذاشتن قوانین برای 

گروه\n#setabout [text]\nگذاشتن متن توضیحات برای سوپر گروه(این متن در بخش توضیحات گروه هم نمایش داده میشه)\n➖➖➖➖➖➖\n#newlink\nساختن لینک جدید\n#link\nگرفتن لینک\n#linkpv\nارسال لینک گروه در پیوی شما!\n➖➖➖➖➖➖\n#rules\nنمایش قوانین\n➖➖➖➖➖➖\n#lock [links|flood|spam|

Arabic|member|rtl|sticker|TgService|contacts|forward|badword|emoji|english|tag|webpage|strict]\nقفل کردن لینک گروها-اسپم-متن و اسم های بزرگ -زبان فارسی-تعداد اعضا-کاراکتر های غیر عادی-استیکر-مخاطبین-فروارد-فوش-اموجی-انگلیسی-تگ-لینک سایت\n\nدقت کنید اگر گذینه اخری 

strict روشن باشد کاربر از گروه کیک میشود و پیغام پاک میشه در غیر این صورت فقط پیغام پاک میشود\n➖➖➖➖➖➖\n#unlock [links|flood|spam|Arabic|member|rtl|sticker|contacts|TgService|strict|forward|badword|emoji|english]\nباز کردن قفل امکانات 

بالا\n➖➖➖➖➖➖\n#mute [all|audio|gifs|photo|video|text]\nپاک کردن سریع همه پیغام ها-عکس ها-گیف ها-صدا های ضبط شده-فیلم-متن\n➖➖➖➖➖➖\n#unmute [all|audio|gifs|photo|video|text]\nباز کردن قفل امکانات بالا\n➖➖➖➖➖➖\n#setflood [value]\nگذاشتن 

value به عنوان حساسیت اسپم\n➖➖➖➖➖➖\n#settings\nنمایش تنظیمات گروه\n➖➖➖➖➖➖\n#muteslist\nنمایش نوع پیغام های سایلنت شده\n*A \"muted\" message type is auto-deleted if posted\n➖➖➖➖➖➖\n#muteuser [username]\nسایلنت کردن یک کاربر خاص در گروه\nفقط 

خریدار (owner) میتونه کسیو سایلنت کنه ولی ناظم ها (Mods) میتونند فرد را از سایلنتی در بیاورند\n➖➖➖➖➖➖\n#mutelist\nنمایش لیست افراد سایلنت شده\n➖➖➖➖➖➖\n#clean [rules|about|modlist|mutelist|bots]\nپاک کردن لیست ناظم ها-درباره-لیست سایلنت شده ها-قوانین-بات ها\n➖➖➖➖➖➖\n#log\nبرگرداندن تاریخچه 

گروه در یک فایل متنی\n➖➖➖➖➖➖\n#version\nنمایش ورژن و ادمین های ربات!\n➖➖➖➖➖➖\n#plist\nدریافت لیست قیمت برای خرید گروه...\n➖➖➖➖➖➖\n#feedback [text]\nشما میتوانید با این دستور نظرات و پیشنهادات خود را برای ما ارسال کنید...\n➖➖➖➖➖➖\n|  |\n"
			reply_msg
...
"user#id"..msg.from.id
		...
		channel_set_admin(receiver, user, ok_cb, false)
				...
				if is_support(msg.from.id) and not is_owner2
...
user = "user#id"..msg.action.user.id
					savelog(msg.to.id, name_log.." Admin ["..msg.action.user.id.."] added to the SuperGroup by [ 
...
is_support(msg.action.user.id) and not is_owner2(msg.action.user.id) then
					local receiver = get_receiver(msg)
			

		local user = "user#id"..msg.action.user.id
					savelog(msg.to.id, name_log.." Support member 

["..msg.action.user.id.."] added to the SuperGroup by [ "..msg.from.id.." ]")
					channel_set_mod(receiver, user, ok_cb, false)
